...
use crate::{
    ...
    indexer::Indexer,
    primitives::{TxMeta, TxMetaVariant},
...
pub struct Server {
    indexer: Box<Arc<dyn Indexer>>,
    satoshi_addr_prefix: &'static str,
    tokens_addr_prefix: &'static str,
}

impl Server {
    pub async fn setup(indexer: Box<Arc<dyn Indexer>>) -> Result<Self> {
        Ok(Server {
            indexer,
...
impl Server {
    ...
    pub async fn tx(&self, tx_hash_str: &str) -> Result<impl Reply> {
        ...
        let is_token = if &title == "eCash Transaction" { true } else { false };
